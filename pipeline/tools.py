...
def _get_recent_news(ticker: str, max_items: int = 5):
    """
    Params:
...
def get_recent_news(ticker: str, max_items: int = 5):
    ...
    return _get_recent_news(ticker, max_items)

def _get_dividend_history(ticker: str): 
    """
    Params: 
...
def get_dividend_history(ticker: str): 
    ...
    To complete
    """
    return _get_dividend_history(ticker) 

def _get_analyst_recommendation(ticker: str):
    """
    Get the analyts recommandation about a specific ticker
    """
    df_recom = yf.Ticker(ticker).get_recommendations_summary()
    if df_recom.empty: 
        return "No recommandations from analysts for this ticker"

    recoms = df_recom.to_dict(orient="records")
...
def get_analyst_recommendation(ticker: str):
    ...
    To complete
    """
    return _get_analyst_recommendation(ticker)
